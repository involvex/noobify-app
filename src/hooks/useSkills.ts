...
export function useSkills() {
  ...
  const loadSkills = useCallback(async () => {
    try {
      ...
      const overrideMap = new Map(overrides.map((o) => [o.term_name, o.enabled === 1]));
  ...
  const toggleSkill = useCallback(async (termName: string, enabled: boolean) => {
    if (enabled) {
    ...
    setCategories((prev) =>
      prev.map((cat) => ({
        ...cat,
        terms: cat.terms.map((t) => (t.name === termName ? { ...t, enabled } : t)),
      })),
    );
  }, []);
  ...
  const customCount = categories.find((c) => c.category === 'custom')?.terms.length ?? 0;
