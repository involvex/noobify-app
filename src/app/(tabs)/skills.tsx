...
export default function SkillsScreen() {
	...
	const handlePressSkill = useCallback(
		(term: TermDefinition, isCustom: boolean) => {
			...
			>
				...
				>
					<View style={styles.statItem}>
						...
						<Text style={[styles.statLabel, {color: colors.onSurfaceMuted}]}>
							Total
						</Text>
					</View>
					<View
				...
				{categories.length === 0 && (
					<View style={styles.emptyContainer}>
						...
						<Text
							style={[styles.emptySubtitle, {color: colors.onSurfaceMuted}]}
						>
							{searchQuery
								? 'Try a different search term'
								: 'Add custom skills to get started'}
						</Text>
					</View>
				)}
			</ScrollView>
			...
			<SkillDetailModal
				...
				enabled={
					detailModal.term
						? (categories
								.flatMap(c => c.terms)
								.find(t => t.name === detailModal.term?.name)?.enabled ?? true)
						: true
				}
				onDismiss={() =>
					setDetailModal({visible: false, term: null, isCustom: false})
