			...
			<Card.Content>
				<View style={styles.cardHeader}>
					<View style={styles.cardHeaderLeft}>
						...
						<Text style={[styles.termText, {color: colors.onSurface}]}>
							{item.original_term}
						</Text>
					</View>
					{!isSelectionMode && (
				...
				<Text style={[styles.analogyText, {color: colors.onSurfaceMuted}]}>
					{item.analogy}
				</Text>
...
export default function HistoryScreen() {
	...
	const [filterLanguage, setFilterLanguage] = useState<'all' | 'en' | 'de'>(
		'all',
	)
	...
	const filteredHistory = useMemo(() => {
		return history.filter(item => {
			const matchesSearch = item.original_term
				.toLowerCase()
		...
		async (item: HistoryItem) => {
			...
			if (isFav) {
				const favToRemove = favorites.find(
					f => f.original_term === item.original_term,
				)
				if (favToRemove) {
